...
bool VvcDecoderConfigurationRecord::ParseInternal() {
  ...
  if (!layered_) {
    RCHECK(reader.Read1(&version_) && version_ == 1 &&
           ...
           reader.ReadToVector(&general_constraint_indicator_flags_, 7) &&
           reader.Read1(&general_level_idc_) &&
           reader.SkipBytes(12) &&  // Skip uninterested fields.
  ...
  for (int i = 0; i < num_of_arrays; i++) {
    ...
    for (int j = 0; j < num_nalus; j++) {
      ...
      Nalu nalu;
      RCHECK(nalu.Initialize(Nalu::kH266, data() + nalu_offset, nalu_length));
      RCHECK(nalu.type() == nal_unit_type);
      AddNalu(nalu);

      if (nalu.type() == Nalu::H266_VPS_NUT ) {
        int vps_id = 0;
        RCHECK(parser_->ParseVps(nalu, &vps_id) == H266Parser::kOk);
      } else if (nalu.type() == Nalu::H266_SPS_NUT) {
        int sps_id = 0;
        RCHECK(parser_->ParseSps(nalu, &sps_id) == H266Parser::kOk);
...
bool VvcDecoderConfigurationRecord::ParseVVCConfig(
